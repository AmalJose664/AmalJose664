...
def justify_format(root, element_id, new_text, length=0):
    ...
    if just_len <= 2:
        ...
        dot_string = ' ' + ('.' * just_len) + ' '
    # print(f"replacing {element_id}_dots, {dot_string}, max = {max(0, length - len(new_text))}, new text len {len(new_text)}, minux = {length - len(new_text)}, receie len = {length}")
